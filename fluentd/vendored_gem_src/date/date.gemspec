# -*- encoding: utf-8 -*-
# stub: date 3.3.3 ruby lib
# stub: ext/date/extconf.rb

Gem::Specification.new do |s|
  s.name = "date".freeze
  s.version = "3.3.3"

  s.required_rubygems_version = Gem::Requirement.new(">= 0".freeze) if s.respond_to? :required_rubygems_version=
  s.require_paths = ["lib".freeze]
  s.authors = ["Tadayoshi Funaba".freeze]
  s.date = "2022-12-19"
  s.description = "A subclass of Object includes Comparable module for handling dates.".freeze
  s.email = [nil]
  s.extensions = ["ext/date/extconf.rb".freeze]
  s.files = ["README.md".freeze, "ext/date/date_core.c".freeze, "ext/date/date_parse.c".freeze, "ext/date/date_strftime.c".freeze, "ext/date/date_strptime.c".freeze, "ext/date/date_tmx.h".freeze, "ext/date/extconf.rb".freeze, "ext/date/prereq.mk".freeze, "ext/date/zonetab.h".freeze, "ext/date/zonetab.list".freeze, "lib/date.rb".freeze]
  s.homepage = "https://github.com/ruby/date".freeze
  s.licenses = ["Ruby".freeze, "BSD-2-Clause".freeze]
  s.required_ruby_version = Gem::Requirement.new(">= 2.6.0".freeze)
  s.rubygems_version = "3.3.26".freeze
  s.summary = "A subclass of Object includes Comparable module for handling dates.".freeze
end

